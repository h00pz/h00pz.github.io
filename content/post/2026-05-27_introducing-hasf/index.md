...
The previous post ended with a system I threw away and two things that replaced it. This is the first of those two: the framework. I have been referring to it obliquely since the very first post, promising it was a subject for another day. This is that day.

It is called HASF, the h00pz Architecture Specification Framework, and the plainest way to describe it is this: it is a way of deciding what a system is before you build it, so that its boundaries, its ownership, and its failure states are chosen on purpose rather than discovered at runtime. PortfolioOS v2 was discovered at runtime. HASF exists so that never has to happen again.

I want to be careful about what this post is. HASF is a large document, and this is not a tour of it. It is an argument for why it exists and the handful of ideas that do most of the work. The mechanics (the failure ledger, the desired-versus-as-built split, cross-reading, spec-before-code, and the definition of done) each get their own post later in this series. Here I only want to convince you that specifying a system first is a real discipline with real rules, not a synonym for drawing a diagram before you code. Where the document's own words land harder than my summary of them, I will quote them directly.

## Most Frameworks Describe Systems That Already Make Sense

Most architecture frameworks are good at describing a system after its architecture is already understood. They give you components, containers, deployment views, runtime interactions, interfaces, technology choices. All of that is necessary, and none of it catches the failures that actually hurt in an AI system, because those failures are not about whether services can talk to each other.

The failures that hurt are semantic and operational. The system answers the wrong question correctly. A model's interpretation quietly becomes more authoritative than the source it came from. Current state and historical state get mixed until nobody can say what is true now. Two stores each look canonical. A retry duplicates work. The interface shows a conclusion that was superseded an hour ago. An operator cannot tell why a result exists. A background job finishes without producing an honest outcome. A system fabricates confidence, or freshness, or completeness, and nothing catches it because everything it did was locally valid.
...
That is the order the framework insists on. Here is the one it rejects, and the contrast is the entire argument:
...
The document does not leave this to interpretation. It writes the preference out as two facing lists:
...
The last rule I will pull out here is the one that keeps the framework from becoming the thing it replaced. HASF is comprehensive in what it asks you to decide and deliberately minimal in what it asks you to build. One state machine. One canonical read model. One authority ladder. One bounded retry. One honest unavailable state. It explicitly warns against duplicate concepts, generalized platforms built before there is a need, arbitrary configurability, and architecture that exists only to compensate for semantics nobody bothered to define.

That last phrase is v2's epitaph. A great deal of what I threw away was architecture that existed only to compensate for unclear meaning. HASF treats simplicity as a governing constraint precisely so that the framework for avoiding accidental complexity does not itself become a generator of it. Comprehensive about the thinking; ruthless about the building.

## Rules Govern Behavior, Repository Artifacts Govern Truth

There is one more idea worth stating, because it is what makes the whole thing survive contact with AI coding agents. HASF separates how work is performed from what the project says is true. The operating instructions an agent follows stay short, stable, and behavioral. The truth of the system (its architecture, its decisions, its state, and its history) lives in canonical, versioned, and linked artifacts in the repository, not in a prompt and not in a conversation.

The framework compresses this whole idea into a single governing line. It is the sentence I come back to most:

> Rules govern behavior. Repository artifacts govern truth.

An implementation agent should not need a forty-page prompt stuffed with copied architecture, component inventories, and current state, because all of that goes stale the moment it is copied. The rules stay small and enforceable; the repository carries the complexity. When a rule file and a canonical artifact disagree about what is true, the canonical owner wins and the stale assertion gets corrected in the same change. This is the same principle as keeping state out of the model, applied to the project's own knowledge: the truth lives in something owned, versioned, and queryable, never in something remembered.
