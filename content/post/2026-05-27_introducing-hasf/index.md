...
I should be honest about that name before it does any more damage. HASF has a four-letter acronym and a version number, `v1.2.2-r6`, which together make it sound like something a standards committee fought over for two years in a hotel conference room. It is a markdown file. One person wrote it, that person is me, and the sum total of its peer review is that I kept using it and it kept working. Even the acronym isn't mine. I described what I wanted to a model, it started calling the thing HASF, I never corrected it, and now here we are: a governing framework for building AI systems, named by an AI, worn by a human who just went along with it. Take it exactly as seriously as that provenance suggests. It's one person's scar tissue with a logo, and the only reason it earns a capital letter is that it has, so far, kept me from rebuilding v2.

I want to be careful about what this post is. HASF is a large document (the <a href="https://github.com/h00pz/h00pz.github.io/blob/main/examples/hasf.md" target="_blank" rel="noopener">whole framework lives as a plain file in this site's repo</a>), and this isn't a tour of it. It's an argument for why it exists and the handful of ideas that do most of the work. The mechanics (the failure ledger, the desired-versus-as-built split, cross-reading, spec-before-code, and the definition of done) each get their own post later in this series. Here I only want to convince you that specifying a system first is a real discipline with real rules, not a synonym for drawing a diagram before you code. Where the document's own words land harder than my summary of them, I will quote them directly.
