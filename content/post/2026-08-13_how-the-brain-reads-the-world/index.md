...
This is the sharp edge of a point Jason Liu keeps making about retrieval, that <a href="https://jxnl.co/writing/2025/01/24/systematically-improving-rag-applications/" target="_blank" rel="noopener">RAG is far more than embedding search</a>: the quality of what you retrieve is decided long before the search runs, in how you extract and structure the unit in the first place. A claim stored as a naked sentence is a bad unit no matter how good the model that later reads it is, and a claim stored as one complete, typed fact is a good one. Most of the fabrication downstream turned out to be an extraction problem wearing a generation problem's clothes.
