...
Once you separate those, done stops being something you assert and becomes something you prove. In the ledger that tracks pOS, a feature is only allowed to call itself live once its acceptance has been demonstrated against the actually running system, through a real read against the deployed API and the real stores, with the command and its output recorded. A green test suite doesn't earn it, because a test that shares the code's assumptions will cheerfully agree with the code and disagree with reality, which means it's checking that I wrote what I wrote, not that the system does what it claims.
...
The payoff of making done this explicit is that the system can start checking its own doneness instead of relying on me to remember. pOS has a test whose entire job is to reconcile the architecture with the code: it fails if any endpoint is mounted that nobody declared, and it fails if any endpoint that was specified and built is wired to nothing. It even proves it can catch both, by injecting each failure and confirming the guard fires, so a green result actually means something rather than being vacuously true.
