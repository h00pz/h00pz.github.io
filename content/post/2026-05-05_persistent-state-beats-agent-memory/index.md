...
The canonical picture of an agent, Lilian Weng's widely cited <a href="https://lilianweng.github.io/posts/2023-06-23-agent/" target="_blank" rel="noopener">LLM plus memory plus planning plus tools</a>, actually puts an external long-term store in the memory box, so it isn't that the serious version of the field wants everything crammed into the context window. Where I part ways is ownership. That framing still calls it the agent's memory, a faculty the agent has, and I want the durable knowledge to belong to the system, with the model as one replaceable reader of it. Same external store, opposite answer to whose it is, and the whole rest of this post is what follows from that flip.
