...
The system was not wrong. It was right, and nothing heard it. That sentence is the whole reason v2 is gone.
...
The `null` monitoring state was not a bug in the sense of a line of code being wrong. Every component involved was locally correct. The pipeline correctly produced findings. The store correctly saved them. The positions correctly existed. What was missing was the join between them, the seam where a changed thesis was supposed to reach the position that depended on it. Nobody had built that seam, because nobody had been forced to say it existed. Each part did its job and the system as a whole did not do its job, and there was no single place to point at, because the failure lived in the space between the parts. That space is exactly where I had never made anything explicit.

## Modules With No Seams

Here is how v2 got that way, and it did not feel like a mistake at any step. You build a component. It needs data from somewhere, so it reaches for it: a database table, a shared library, or another module's internals. That reach works. It is now real, undocumented, and load-bearing. Nothing ever made you declare it, so it stays invisible.
...
The result is a system that looks decomposed on a diagram and behaves like one tangled object. You cannot change a piece without discovering, at runtime, the six other pieces that were quietly depending on its insides. The monitoring failure was one instance of a general condition: v2 had no seams, so v2 had no boundaries, so v2 could not be reasoned about a piece at a time.
...
The deeper issue with all those safeguards was that most of them had lost their reasons. A rule would be added in response to some specific incident, the incident would fade from memory, and what remained was a constraint nobody could explain. And a rule whose reason has been forgotten has only two fates: it gets deleted the first time it is inconvenient, or it gets worked around, quietly, by the next person who hits it and cannot see why it is there.

Neither of those is the outcome you want. What you want is for the rule to be *argued*: for someone to look at it, see exactly which failure it prevents, and decide on the merits whether that failure still matters. That is only possible if the reason was recorded next to the rule. v2's rules were not recorded that way. They were just there, a sediment of past fears, and I could no longer tell which ones were load-bearing and which were superstition.
...
Throwing away working software is expensive and slightly embarrassing, and I would not have done it for a cleaner diagram. I did it because v2 had taught me, concretely and at cost, what happens when integration is implicit, when safeguards outrun their reasons, and when a system cannot see its own state. Those are not problems you patch. They are problems you design against from the beginning, or you inherit them.

The rest of this series is the design that came out of that. Two things replaced v2, and they are the subjects of the next two posts. The first is a framework, a way of specifying a system before building it, so that boundaries, ownership, and failure states are decided on purpose rather than discovered at runtime. The second is a method, the actual, lived, and occasionally humiliating process of using that framework to build a subsystem, including all the ways I got it wrong.

Both exist for the same reason. v2 worked right up until I asked it what it believed and it could not tell me. I did not want to build another system that could only be trusted until the first hard question. The answer to why we threw away PortfolioOS v2 is that it was easier to rebuild it around the failures I could name than to keep defending it against the ones I could not.

That is the whole of Phase 2: the scars, and what I built because of them. Both of those are the subject of everything that follows.
